...
# 테스트해보니(자매 저장소 OSMU_thestitch에서 먼저 재현됨), 이어서
# 작성하시겠습니까 팝업은 .se-popup-button-cancel로 attempt 1에서 바로
# 닫혔는데, 그 뒤로도 범위 없는 button:has-text('취소')가 페이지 어딘가의
# *다른*(팝업과 무관한) "취소" 버튼을 계속 찾아내는 바람에 20번을 다 돌
# 때까지 "아직 팝업이 있다"고 오판했다 — 결국 타임아웃되거나 사용자가
# 아무 반응이 없다고 여겨 Chrome 창을 닫아버려 TargetClosedError로 게시가
    ...
    with TemporaryDirectory() as tmp_dir_str:
        ...
        try:
            with sync_playwright() as p:
                ...
                # (sister repo OSMU_thestitch) showed the title ending up
                # missing and the first paragraph of body text landing
                # partially or not at all, with an image showing up first
                # instead — consistent with this Ctrl+A/Delete not staying
                # scoped to just the body (Smart Editor ONE's title and body
                # are components inside the same document, and a "select
                # all" issued a beat after the title was typed can catch
                # more than intended) and wiping content that was already
                # typed in. Clearing the body *before* anything meaningful
                # has been entered anywhere means there is nothing upstream
                # left for an over-broad select-all to destroy.

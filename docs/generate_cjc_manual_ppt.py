...
범위: LLM/네이버 API 키 등록 → SEO 키워드 새로 고르기·수치 유지 → 일상 사용법 → 주의사항·FAQ.
...
add_text(s, Inches(0.9), Inches(4.15), Inches(10.5), Inches(0.9),
         "API 키 등록부터 SEO 키워드 고르기·유지, 일상 사용법과 주의사항까지",
...
parts = [
    ...
    ("핵심", "② SEO 키워드 새로 고르기와 수치 유지", "이 매뉴얼의 심장부: 씨앗→발굴→측정→판정→적용, 그리고 30일마다 숫자 재기", PRIMARY),
...
rows = [
    ...
    ("🔑 SEO 키워드 새로 고르기", "씨앗(🪄 추천) → 1~4단계. 4단계 [적용]이 곧 브랜드 킷 SEO 키워드 교체", "처음 1회 · 이후 분기 1회 (30분)"),
    ("🔄 키워드 수치 유지", "[🔄 숫자만 새로 재기] — 목록은 그대로, 만료된 수치만 다시 측정", "30일마다 (5분)"),
...
add_text(s, Inches(0.85), Inches(5.55), Inches(11.6), Inches(0.9),
         ...
         "데이터로 판단하는 SEO 키워드 새로 고르기와 수치 유지뿐입니다.",
...
add_bullets(s, Inches(0.55), Inches(1.5), Inches(6.0), Inches(4.5), [
    (0, "🔑 SEO 키워드 새로 고르기와 🔄 수치 유지가 활성화됩니다.", True),
    (0, "키가 없을 때는 경고 한 줄만 보이고 [1~4단계]·[숫자만 새로 재기] 버튼이 잠겨 있습니다.", False),
...
# ============================================================
s = new_slide()
header_bar(s, "② SEO 키워드 새로 고르기 · 씨앗", "씨앗 키워드란 — 그물의 첫 한 땀")
...
# ============================================================
s = new_slide()
header_bar(s, "② SEO 키워드 새로 고르기 · 실행", "[🪄 추천]과 4단계 파이프라인")
...
steps5 = [
    ...
    ("4. 적용", PAID_TAG, "API HUB", "이 순간 브랜드 킷 SEO 키워드가 바뀝니다 + 새 키워드 측정까지 완료 — 따로 할 일 없음"),
]
y = Inches(2.9)
...
IMG = "docs/"
s = new_slide()
header_bar(s, "② SEO 키워드 새로 고르기 · 실제 화면 (1/3)", "씨앗 입력 → 1단계 · 후보 찾기")
add_screenshot(s, IMG + "naver_kw_01_start.png", Inches(0.55), Inches(1.5), Inches(5.85),
               caption="위 🔑 새로 고르기(씨앗→1~4단계), 아래 🔄 수치 유지 — 처음엔 [1단계]부터")
...
# ============================================================
s = new_slide()
header_bar(s, "② SEO 키워드 새로 고르기 · 실제 화면 (2/3)", "2단계 · 경쟁도 조사 결과")
...
# ============================================================
s = new_slide()
header_bar(s, "② SEO 키워드 새로 고르기 · 실제 화면 (3/3)", "3단계 · 브랜드 판정 → 4단계 · 적용")
...
# ============================================================
s = new_slide()
header_bar(s, "② 🔄 키워드 수치 유지", "30일마다 5분 · 분기 30분 · 캐시 30일")
colw = Inches(5.95)
add_rect(s, Inches(0.55), Inches(1.5), colw, Inches(3.2), WHITE)
add_text(s, Inches(0.8), Inches(1.65), colw - Inches(0.5), Inches(0.4), "30일마다 — [🔄 숫자만 새로 재기]", size=14, color=PRIMARY, bold=True)
add_text(s, Inches(0.8), Inches(2.1), colw - Inches(0.5), Inches(2.4),
         ...
         "4단계 [적용] 직후에는 누를 필요가 없습니다(버튼이 '전부 최신 · 0회'로 잠겨 있음).",
         size=12, color=TEXT, line_spacing=1.35)
add_rect(s, Inches(6.85), Inches(1.5), colw, Inches(3.2), WHITE)
add_text(s, Inches(7.1), Inches(1.65), colw - Inches(0.5), Inches(0.4), "분기 — 🔑 SEO 키워드 새로 고르기", size=14, color=PRIMARY, bold=True)
...
warn_items = [
    ...
    ("키 없이 눌렀을 때 증상 3종", "'기본 생성 모델이 지정되지 않았습니다'(🪄 추천) · [1~4단계]·[숫자만 새로 재기] 버튼 잠김 · 경쟁도 만료 경고 — 모두 키 등록으로 해결."),
...
faqs2 = [
    ...
    ("4단계 적용 후에 [숫자만 새로 재기]를 또 눌러야 하나요?", "아니요. 4단계 [적용]이 곧 SEO 키워드 교체이고, 새 키워드 측정까지 같이 끝냅니다. 숫자 재기는 30일 뒤 만료 경고가 뜰 때만."),
